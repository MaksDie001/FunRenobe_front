            ...
            {this.state.Isloading ?<div >
                    ...
                    <div className={"row renobe_info"}>
                        ...
                        <div className={"col-8 row"}>
                            <div className={"col-6"}>
                                <div className={"date icon_label"}><span className={"icons_span"}><AiOutlineClockCircle className={"icons"}/></span> <span className={"icons_description"}>Было добавлено:</span></div>
                                <div className={"icon_text"}>{renobe.date_join}</div>

                                <div className={"author icon_label"}><span className={"icons_span"}><BiPencil className={"icons"}/></span> <span className={"icons_description"}>Автор:</span></div>
                                <div className={"icon_text"}>{renobe.writer_user_id}</div>

                                <div className={"tags icon_label"}><span className={"icons_span"}><FiBook className={"icons"}/></span> <span className={"icons_description"}>Жанры:</span></div>
                                <div className={"tags"}><Convert_compoent text={this.state.renobe.tags} text_lenght={JSON.stringify(this.state.renobe.tags).length} max_lenght={40}  /></div>
                            </div>
                            <div className={"col-6"}>
                                <div className={"date icon_label"}><span className={"icons_span"}><FiHeart className={"icons"}/></span> <span className={"icons_description"}>Ркйтинг Рэнобе:</span></div>
                                <div className={"icon_text"}>
                                    <FiHeart className={"like"} />
                                    <RiDislikeLine className={"dislike"} />
                                </div>

                                <div className={"date icon_label"}><span className={"icons_span"}><BsGeoAltFill className={"icons"}/></span> <span className={"icons_description"}>Страна:</span></div>
                                <div className={"icon_text"}><Country country_id={renobe.country} /></div>
                            </div>
                            <div className={"row descriptions_label"}>
                                <div  className={`col-6 description ${this.state.description_color_status1}`} id={"description_i"} onClick={()=>this.description_controller1()}>Описание<hr/></div>
                                <div  className={`col-6 description ${this.state.description_color_status2}`} onClick={()=>this.description_controller2()}>Информация а рэнобе<hr/></div>
                                {this.state.description_status ?<Convert_compoent class={"renobe_title"} text={this.state.renobe.renobe_title} text_lenght={JSON.stringify(this.state.renobe.renobe_title).length} max_lenght={100} />:
                                    <div className={"icon_text"}><div className={"renobe_description"}>
                ...
                <div className={"chapter_list_container"}>
                    ...
                    {this.state.chapter_list.map(res=>{
                        return(
                            <div key={res.id} className={"chapter_list"}> Глава номер:{res.chapter_number}.{res.chapter_title}</div>
                        )
                    })}
                </div>
                </div>:<Spiner />}
